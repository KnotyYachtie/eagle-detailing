...
5. **Site footer (`Footer.astro`)** — **Responsive CSS Grid**: **desktop (`≥1024px`)** four columns — brand (logo + tagline + Instagram), **Explore** (`NAV_SERVICES` + `NAV_MAIN` only), contact (`tel` / `mailto` / locality), **Get started** (`/contact` + `sms:` secondary); **tablet (`768–1023px`)** `2×2` areas (`brand | cta` / `explore | contact`); **mobile (`<768px`)** stacked **brand → CTA → Explore (two-column link grid) → contact**, **column content horizontally centered** (`text-align`, flex/grid alignment); then **bottom legal** row centered on narrow viewports, **`≥768px`** legal uses row layout (`©` vs locality + licensed). Footer background is **flat** `var(--footer-bg)` (no gradient overlays).  
   *Plain English:* Same footer columns and phone stacking order as before, but on small screens everything lines up to the middle; wider screens keep the footer’s side-by-side legal row and tablet/desktop alignment.

6. **Service vertical shell (`ServiceEditorialPage.astro`)** — **`/marine`**, **`/automotive`**, and **`/aviation`** share one editorial layout (**`marine-page`** wrapper + hero stack + chapters + featured strip + quote + services grid + reach form) and one **`SERVICE_EDITORIAL_SHELL_COLORS`** pair in **`src/site.ts`** (`themeColor` / `canvasColor`, currently **`#0a1118`**) passed into **`BaseLayout`** with **`transparentHeader={true}`** and **`dataHomeLoadIntro={true}`**. **Marine** supplies imagery (parallax hero, chapter photos, Flowbite carousel, inline eagle SVG) and a **`services-rows`** slot with the full icon list. **Automotive** / **aviation** omit images for now: gradient **`marine-page__parallax-fill`**, dashed **`marine-chapter__frame--placeholder`**, **`marine-window__placeholder`** instead of carousel, dashed **`marine-services__eagle--placeholder`**, and generic **`serviceRows`** copy. **`marine-reach-form.ts`** binds every **`form[data-eagle-service-reach]`** and reads **`data-reach-subject-prefix`** for mailto subjects.  
   *Plain English:* The three service routes share one template and the same navy browser/chrome tint; Marine has real photography, while Automotive and Aviation use gray placeholders until assets ship.
...
27. **Assumption:** **v1 contact** without a form—**phone-first**, email, Instagram—is intentional and not a conversion liability for this clientele.  
    *Plain English:* Rich clients are expected to call or DM, not fill out a web form on day one.  
