...
3. **Header chrome** — Default **`transparentHeader={true}`**: **transparent** sticky bar + **vertical brand lockup** (`assets/branding/logo-lockup-transparent.png`) over hero photography — **no** `logo-light.png`. **`transparentHeader={false}`** (e.g. `/contact`): **flat** `#050a14` bar, **no** navy glass/blur slab — same PNG lockup. **Favicon:** eagle mark on **`#102135`** (`logo-eagle-transparent.png` composited in `public/favicon*.png` + `favicon.ico`); not the full text lockup at 32px. **`SITE.faviconVersion`** cache-bust on all icon links; **`favicon.svg` removed**.  
   *Plain English:* Header uses one stacked transparent logo image (roughly 2× prior width); tab icon is a crisp eagle on navy without white edge artifacts.
