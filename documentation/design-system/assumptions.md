...
3. **Header chrome** — Default **`transparentHeader={true}`**: **transparent** sticky bar + **horizontal brand lockup** (`assets/branding/logo-eagle-transparent.png` + `logo-text-transparent.png`) over hero photography — **no** `logo-light.png`. **`transparentHeader={false}`** (e.g. `/contact`): **flat** `#050a14` bar, **no** navy glass/blur slab — same PNG lockup. **Favicon:** `public/favicon.png` (32×32) + `public/apple-touch-icon.png` from client navy lockup.  
   *Plain English:* Most pages float the nav over the hero with the new eagle + wordmark images side by side; solid-header pages use the same mark on a dark strip. Browser tab icon uses the navy full lockup.
