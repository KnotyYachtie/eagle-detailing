...
3. **Header chrome** — Default **`transparentHeader={true}`**: **transparent** sticky bar + **vertical brand lockup** (`assets/branding/logo-lockup-transparent.png`) over hero photography — **no** `logo-light.png`. **`transparentHeader={false}`** (e.g. `/contact`): **flat** `#050a14` bar, **no** navy glass/blur slab — same PNG lockup. **Favicon:** `public/favicon.ico` + PNG sizes (`16` / `32` / `48`) + `apple-touch-icon.png` from client navy lockup; **`SITE.faviconVersion`** cache-bust query on all `<link rel="icon">` tags; legacy **`favicon.svg` removed**.  
   *Plain English:* Header uses one stacked transparent logo image; browser tab icons are real PNG/ICO from the navy lockup with cache-busting so updates actually show up.
