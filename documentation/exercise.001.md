...
*Last updated: **rev. 3 + spine + poses (Marine = Automotive lateral)** — domains as **three scroll chapters** on canvas (**`domains-drift.ts`** + **`--drift-exit`**); **timeline spine**: **§ F** (ship bar vs first ship vs landed; spine links **row-centered** beside chapters), **§ G** (root causes), **§ H** (chronology), **§ I** (forward checklist + **`spine-layout.spec.ts`**); **§ Current** — **Aviation** zig layout; **Marine** + **Automotive** share lateral pose; rev. 2 runway archived above; **sandbox** documents **gaps to prevent repeat oversights**.*
